	...
	if(isset($RowID)){
		echo '<form class="record_unlock" name="record_unlock">';
			...
			echo '<div class="text-center">';
				echo '<button type="button" class="btn btn-warning b_i f_16" id="submit_unlock"><i class="fa fa-check-circle"></i> Un-lock Setting</button>';
			echo '</div>';
		echo '</form>';
	}
?>
<script>
	$(document).ready(function(){
		$("button#submit_unlock").click(function(){
			$.ajax({
				type: "POST",
				url:"<?php echo $Style->_style_array[9];?>AJAX/AP/Settings/settings_access_submit.php",
