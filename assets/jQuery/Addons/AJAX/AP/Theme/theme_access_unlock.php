<?php
	...
	if(isset($_POST['id'])){
		echo '<form class="theme_unlock" name="theme_unlock">';
			...
			echo '<div class="text-center f_20">';
				echo '<button type="button" class="badge badge-warning" id="theme_unlock_submit"><i class="fa fa-check-circle"></i> Un-lock Setting</button>';
			echo '</div>';
		echo '</form>';
	}
?>
<script>
	$(document).ready(function(){
		$("button#theme_unlock_submit").click(function(){
			$.ajax({
				type: "POST",
				url:"<?php echo $Style->_style_array[9];?>AJAX/AP/Theme/theme_access_submit.php",
				data: $('form.theme_unlock').serialize(),
				success: function(message){
					$('#theme_unlock_modal #dynamic-content').html(message);	// load response
