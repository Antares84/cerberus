	...
	if(isset($_POST['id'])){
		echo '<form class="record_lock" name="record_lock">';
			...
			echo '<div class="text-center f_20">';
				echo '<button type="button" class="badge badge-primary" id="submit_lock"><i class="fa fa-check-circle"></i> Update Setting</button>';
			echo '</div>';
		echo '</form>';
	}
?>
<script>
	$(document).ready(function(){
		$("button#submit_lock").click(function(){
			$.ajax({
				type: "POST",
				url:"<?php echo $Style->_style_array[9];?>AJAX/AP/Styles/access_submit.php",
				data: $('form.record_lock').serialize(),
				success: function(message){
					$('#lock_modal #dynamic-content').html(message);
