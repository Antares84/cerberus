<?php
	# Plugin Information
	$this->MODULE_NAME			=	'DropFinder';
	$this->MODULE_MASTERFILE	=	'module.DropFinder.php';
	$this->MODULE_PHP			=	'DropFinder.php';
	$this->MODULE_AJAX			=	'search.php';
	$this->MODULE_JS			=	'modal.DropFinder.js';
	$this->MODULE_VERSION		=	'1.0';
	$this->MODULE_DATE			=	'09.16.2017';
	...
	elseif($this->MODE == "DISPLAY"){
		...
		if($prep){
			while($data=odbc_fetch_array($stmt)){
				if($data["MODULE_PHP"]){
					require_once($this->Dirs->_arr["MODULES"].$data["MODULE_NAME"]."/php/".$data["MODULE_PHP"]);
				}
				if($data["MODULE_AJAX"]){
					require_once($this->Dirs->_arr["MODULES"].$data["MODULE_NAME"]."/ajax/".$data["MODULE_AJAX"]);
				}
				if($data["MODULE_JS"]){
					require_once($this->Dirs->_arr["MODULES"].$data["MODULE_NAME"]."/js/".$data["MODULE_JS"]);
