<?php
	...
	if(isset($_POST["RowID"])){
		...
		$sql	=	('
						UPDATE '.$db->get_TABLE("SETTINGS_STYLE").'
						SET VALUE=?
						WHERE RowID=?
		');
		...
		if($prep){
			$Tpl->BADGE_AJAX('badge-success','<i class="fa fa-info-circle"></i> Style data successfully updated.');
		}else{
			$Tpl->BADGE_AJAX('badge-danger','<i class="fa fa-info-circle"></i> Style data update failed.');
		}
	}
	else{
