<?php
	...
	if(isset($_POST) && !empty($_POST)){
		# ACCOUNT
		$DisplayName	=	isset($_POST["DisplayName"])	?	$Data->_do('escData',trim($_POST["DisplayName"]))	:	false;	#0
		$UserID			=	isset($_POST["UserID"])			?	$Data->_do('escData',trim($_POST["UserID"]))		:	false;	#1
		$Password		=	isset($_POST["Password"])		?	$Data->_do('escData',trim($_POST["Password"]))		:	false;	#2
		$c_Password		=	isset($_POST["c_Password"])		?	$Data->_do('escData',trim($_POST["c_Password"]))	:	false;	#3
		$Referer		=	isset($_POST["Referer"])		?	$Data->_do('escData',trim($_POST["Referer"]))		:	false;	#4
		# PERSONAL
		$DOB			=	isset($_POST["DOB"])			?	$Data->_do('escData',trim($_POST["DOB"]))			:	false;	#5
		$Gender			=	isset($_POST["Gender"])			?	$Data->_do('escData',trim($_POST["Gender"]))		:	false;	#6
		$SecQuestion	=	isset($_POST["SecQuestion"])	?	$Data->_do('escData',trim($_POST["SecQuestion"]))	:	false;	#7
		$SecAnswer		=	isset($_POST["SecAnswer"])		?	$Data->_do('escData',trim($_POST["SecAnswer"]))		:	false;	#8
		$EMail			=	isset($_POST["EMail"])			?	$Data->_do('escData',trim($_POST["EMail"]))			:	false;	#9
		# TOS
		$Checkbox		=	isset($_POST["Checkbox"])		?	$Data->_do('escData',trim($_POST["Checkbox"]))		:	false;	#10
		# MISC
		$UserIP			=	$Browser->UserIP;																					#11
		$ActivationKey	=	$Data->_do('rand_str');																					#12
		$ResendURI		=	$Setting->_array["SITE_DOMAIN"].'?'.$Setting->_array["PAGE_PREFIX"].'=ResendValidation&Key='.$ActivationKey;			#13
		$ValidateURI	=	$Setting->_array["SITE_DOMAIN"].'?'.$Setting->_array["PAGE_PREFIX"].'=Validate&Key='.$ActivationKey;					#14

		$CheckUser		=	odbc_exec($db->conn,"SELECT UserID,Email FROM ".$db->_table_list('WEB_PRESENCE')." WHERE UserID='".$UserID."'");
		$CheckEmail		=	odbc_exec($db->conn,"SELECT Email FROM ".$db->_table_list('WEB_PRESENCE')." WHERE Email='".$EMail."'");
...
#		$Mailsys->msg_arr[]=
		...
		if(count($err) == 0){
			...
			$REG_WEB	=	$SQL->_do_REGISTER_USER_WEB($RegInfo[1],$RegInfo[2],$RegInfo[0],$RegInfo[5],$RegInfo[6],$RegInfo[4],$RegInfo[7],$RegInfo[8],$RegInfo[12],$RegInfo[9],$RegInfo[11],$RegInfo[11]);
			...
			#	header('location: ?'.$Setting->PAGE_PREFIX.'=REGISTER&Valid=true');
				...
				echo 'Web registration failed...';
			}

			if($Setting->_array["SITE_TYPE"] == 3){
				echo 'In Shaiya reg...';
